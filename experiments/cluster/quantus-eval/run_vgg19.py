...
def load_vgg19_flat(device):
    ...
    m = torch.nn.Sequential(*m.features, m.avgpool, torch.nn.Flatten(), *m.classifier)
...
methods = (
    ...
    GradCAMExplainer(layer_index=34),
...
def run_metric(method, metric_name, metric_fn):
    ...
    dir_path = Path(f"results/{BATCHSIZE}/vgg19/{metric_name}")
